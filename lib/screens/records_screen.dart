...
class _RecordsScreenState extends State<RecordsScreen> with SingleTickerProviderStateMixin {
  ...
  void _generateMonthList() {
    ...
    // Generate months from April 2026 to Now
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => CsvExportService.exportRecordsToCsv(ballProvider.allRecords),
          ),
        ...
        bottom: TabBar(
          ...
          tabs: _monthList.map((m) {
            String display = m == 'Overall' ? 'Overall' : DateFormat('MMMM yyyy').format(DateFormat('MM-yyyy').parse(m));
            return Tab(text: display.toUpperCase());
          }).toList(),
        ),
      ),
  ...
  Widget _buildMonthTable(String monthYear, List<BallRecord> allRecords) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        // Calculate daily total only for active (positive) entries
        for(var name in playerNames) {
          dailyGrandTotal += summarized[name]!;
        }
              ...
              Column(
                children: [
                  ...
                  Container(
                    ...
                    child: Text('Total: $dailyGrandTotal', style: GoogleFonts.bebasNeue(color: Colors.orange, fontSize: 10)),
                  ),
                ],
              ),
                        ...
                        children: [
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(name, style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
